...
import {
  GIFT_HISTORY_CAPABILITY,
  ProviderResolutionError,
  discoverProviders,
  readFromProvider,
  resolveProvider,
  validateGiftHistorySnapshot,
...
export async function resolveGiftSource(args) {
  ...
  const provider = await resolveProvider({
    providers,
    capability: GIFT_HISTORY_CAPABILITY,
    request,
  ...
  const snapshot = validateGiftHistorySnapshot(await readFromProvider(provider, request));
...
export async function main(argv = process.argv.slice(2)) {
  try {
    const result = await resolveGiftSource(parseArgs(argv));
    ...
    console.log(JSON.stringify({
      ...
      providerBinding: result.providerBinding,
      knowledgeVersion: result.knowledgeVersion,
      accountKey: result.snapshot.accountKey,
